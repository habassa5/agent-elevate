...
- **SYSTEM scheduled-task broker** (`AgentElevate-Broker`) that runs admin-curated, allow-listed parameterized
  operations as `SYSTEM` with **no UAC prompt**, plus a **self-heal** task that restores it after reboots and
  Windows updates.
...
- **Elevated installer** `setup-agentelevate.ps1` with SHA-256 source and deployed pinning and fail-closed
...
- `SECURITY.md` (threat model and private vulnerability-reporting policy), MIT `LICENSE`.

### Security
- Trust anchor = **admin-only path ACL plus Administrators ownership** of `C:\Program Files\AgentElevate\` (no
